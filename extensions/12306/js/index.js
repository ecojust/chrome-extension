...
async function kyfw() {
  ...
  const hifiniMusicPluginHtml = `
    ...
    <el-button @click="startCollect(1000)" >重置数据且开始采集</el-button>

    <el-button @click="hide = !hide" >{{hide?'打开':'收起'}}结果</el-button>
        ...
        for (var i = 0; i < cars.length; i++) {
          ...
          for (var j = 0; j < car.stations.length; j++) {
            if (j > 0) {
              ...
              var targeturl = `https://kyfw.12306.cn/otn/leftTicket/init?linktypeid=dc&fs=${fromzh},${fromcode}&ts=${tozh},${tocode}&date=2023-09-28&flag=N,Y,Y`;
              console.log("open------");
              window.open(
                targeturl +
              ...
              await window.XHRTool.sleep(5000);
              this.update();
              if (step % 10 == 0) {
                await window.XHRTool.sleep(60000);
              }
              if (car.to == tocode) {
                break;
              }
