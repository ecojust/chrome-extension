...
async function gitee() {
  ...
  hifiniMusicPlugin.setAttribute("class", "Gitee-Issues-Tool");
  hifiniMusicPlugin.setAttribute("id", "Gitee-Issues-Tool");
  var DOMID = "#Gitee-Issues-Tool";
  const hifiniMusicPluginHtml = `
    ...
    <button @click="startCollect(1000)" >重置数据且开始采集</button>

    <button @click="downloadSheet" >下载sheet</button>


    <h5 v-for="i in messages" :key="i">{{i}}</h5>
    ...
    methods: {
      async startCollect(interval = 1000) {
        ...
        if (
          ...
          this.messages = ["开始获取issues列表..."];
          var data = await getIssues();
          ...
          for (var i = 0; i < size; i++) {
            ((i) => {
              setTimeout(async () => {
                this.messages[3] = `即将解析第${i + 1}条...`;
                this.messages = [...this.messages];

                var w = window.innerWidth - 100,
                  h = window.innerHeight - 100;
                window.open(
                  data[i].path + `?needcollect=true`,
                  "_blank",
                ...
                if (size == i + 1) {
                  this.messages[4] = `全部数据解析完毕...`;

                  this.messages[5] = `3s之后自动下载excel表格...`;
                  this.messages = [...this.messages];
                  await window.XHRTool.sleep(1000);
                  this.messages[5] = `2s之后自动下载excel表格...`;
                  this.messages = [...this.messages];
                  await window.XHRTool.sleep(1000);
                  this.messages[5] = `1s之后自动下载excel表格...`;
                  this.messages = [...this.messages];
                  await window.XHRTool.sleep(1000);
                  this.downloadSheet();
                }
      ...
      autoAnalysis() {
        ...
        if (
          pathname.includes("/issues/") &&
          search.includes("?needcollect=true")
        ) {
          console.log("needcollect");
          var description = document.querySelector(".git-issue-content");

          var dataSource = localStorage.getItem("issues") || "[]";
          dataSource = JSON.parse(dataSource);
          var number = pathname.split("/issues/")[1];
          var item = dataSource.find((i) => i.number == number);
          if (item) {
            item.description = description.innerText;
          }

          var comments = document.querySelectorAll(".single-comment-wrapper");
          var analysis = "";
          comments.forEach((comment) => {
            console.log(comment);
            var name = comment.dataset["name"];
            var time = comment.dataset["updatedAt"];
            time = time.replace("+0800", "");
            var content = comment.querySelector(".content-body").innerText;
            analysis += `【${name}:${time}】\n${content}\n`;
          });
          item.analysis = analysis;

          localStorage.setItem("issues", JSON.stringify(dataSource));
          window.close();
        }
      },
